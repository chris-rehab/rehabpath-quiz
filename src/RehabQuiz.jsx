...
const EJS_SERVICE_ID        = "service_x79o9kf";
const EJS_USER_TEMPLATE_ID  = "template_vk2f1bk";
const EJS_CHRIS_TEMPLATE_ID = "template_wtlgirl";
const EJS_PUBLIC_KEY        = "TXeDbqUiSb3hkV6up";
// ─────────────────────────────────────────────────────────────────────────────
...
// ─── NEXT STEPS CONTENT (per profile) ────────────────────────────────────────
const NEXT_STEPS = {
  driven: {
    headline: "Your drive is real. Let's build the system around it.",
    body: "People with your motivation profile are often just one structural shift away from real consistency. The research shows that the biggest predictor of follow-through isn't willpower — it's having the right prompt at exactly the right moment. RehabPath is built precisely for this: personalised check-ins that adapt to your patterns, not a one-size schedule that ignores them. Join the pilot waitlist and be first to experience it.",
  },
  achiever: {
    headline: "You're primed. Let's make sure that momentum compounds.",
    body: "You're in a strong position — motivated, clear on your goal, and with relatively few barriers in the way. The risk for people at your stage isn't starting, it's the week 3 wall when novelty fades and the routine hasn't yet become automatic. RehabPath's messaging is timed specifically to intervene at that window — keeping momentum alive when it matters most. Join the pilot and protect your progress.",
  },
  explorer: {
    headline: "Small wins beat big commitments every time.",
    body: "The research is unambiguous: starting smaller than feels necessary is what actually builds lasting habit. Your profile suggests there are a few real barriers in the way right now — and that's completely valid. RehabPath is designed to reduce friction at exactly those points: gentle, well-timed prompts that meet you where you are rather than where you think you should be. Join the pilot waitlist and take the first small step.",
  },
  mover: {
    headline: "The path is clearer than it feels right now.",
    body: "Often the biggest barrier isn't motivation — it's visibility. When progress is invisible, it's easy to underestimate how close you actually are. RehabPath makes small wins visible and builds on them, so momentum becomes something you can see and feel rather than something you have to conjure. Join the pilot waitlist and let the system do the heavy lifting.",
  },
};

// ─── EMAILJS SEND ─────────────────────────────────────────────────────────────
async function sendEmails(contactInfo, answers, profile) {
  const firstName   = contactInfo.name?.trim().split(" ")[0] || "";
  const nextSteps   = NEXT_STEPS[profile.key] || NEXT_STEPS.driven;
  const submittedAt = new Date().toLocaleString("en-NZ", { timeZone: "Pacific/Auckland" });

  const sendTemplate = (templateId, params) =>
  ...
  // Email to Chris — always fires
  await sendTemplate(EJS_CHRIS_TEMPLATE_ID, {
    user_name:       contactInfo.name  || "Anonymous",
    user_email:      contactInfo.email || "Not provided",
    profile_type:    profile.type,
    profile_key:     profile.key,
    importance:      answers.importance ?? "—",
    confidence:      answers.confidence ?? "—",
    stage:           stageLabel(answers)   || "—",
    barriers:        (answers.barriers || []).join(", ") || "—",
    miss_frequency:  freqLabel(answers)    || "—",
    after_miss:      recoveryLabel(answers)|| "—",
    main_goal:       goalLabel(answers)    || "—",
    support_needed:  (answers.support || []).join(", ") || "—",
    success_vision:  successLabel(answers) || "—",
    submitted_at:    submittedAt,
  });

  // Email to user — only if they provided an email
  if (contactInfo.email?.trim()) {
    await sendTemplate(EJS_USER_TEMPLATE_ID, {
      to_name:             firstName || "there",
      to_email:            contactInfo.email.trim(),
      profile_type:        profile.type,
      profile_icon:        profile.icon,
      profile_description: profile.description,
      profile_fit:         profile.fit,
      next_steps_headline: nextSteps.headline,
      next_steps_body:     nextSteps.body,
      main_goal:           goalLabel(answers)    || "—",
      success_vision:      successLabel(answers) || "—",
      barriers:            (answers.barriers || []).join(", ") || "—",
    });
...
export default function RehabQuiz() {
  ...
  const handleSubmit = async (skip = false) => {
    ...
    const info = contact;
    try {
      await sendEmails(info, answers, p);
    } catch (err) {
      console.warn("EmailJS error (results still shown):", err.message);
    } finally {
      setSubmitting(false);
  ...
  const renderCapture = () => (
    <div style={fadeStyle}>
      ...
      <div className="section-label">Your results are ready</div>
      <p className="question-text">Want a copy sent to you?</p>
      <p className="capture-sub">
        Your personalised profile is waiting on the next screen. Add your details below
        if you'd like us to send you a copy — or just hit the button to see your results now.
      </p>
      <div className="field-group">
        <div className="field-wrap">
          <label className="field-label">First name <span style={{fontWeight:300,textTransform:'none',letterSpacing:0,color:'#8FA3B4'}}>(optional)</span></label>
          <input
        ...
        <div className="field-wrap">
          <label className="field-label">Email address <span style={{fontWeight:300,textTransform:'none',letterSpacing:0,color:'#8FA3B4'}}>(optional)</span></label>
          <input
      ...
      <p className="privacy-note">
        🔒 If you share your email, it's used only for this pilot. We don't share your information with anyone.
      </p>
